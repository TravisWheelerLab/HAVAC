...
int main() {
  ...
  for (size_t i = 1; i <= numTests; i++) {
    ...
    fflush(stdout);
    std::vector<struct TestbenchHitReport> hardwareSsvHits = invokeHardwareSsv(sequenceAsVectorIndices, hmmSeqPair.sequenceLength,
    	phmmAsUint32, hmmSeqPair.phmmList->phmms[0].header.modelLength, errorCode);
    delete[] phmmAsUint32;
    printf("hardware finished\n");

    std::ofstream outStream("~/softHits.txt", std::ios::trunc);
    outStream << "num hits: "<< hardwareSsvHits.size() << "\n";
    for(auto &hit: hardwareSsvHits){
    	std::cout << "hw hit : p="<< hit.phmmPosition<< ", s = "<< hit.sequencePosition<< std::endl;
    	outStream << "p = "<< hit.phmmPosition<< ", s = "<< hit.sequencePosition<< "\n";
    }
    outStream.close();
...
std::vector<TestbenchHitReport> invokeHardwareSsv(const uint8_t* sequenceAsVectorIndices, size_t sequenceLength,
	uint32_t *emissionsAsUInt32_t, const size_t phmmLength, int8_t& errorCode) {
  errorCode = 0;
  std::vector<TestbenchHitReport> hitReportList;
  ...
  uint64_t* hitReportMemory = (uint64_t*)malloc(sizeof(uint64_t) * NUM_HIT_REPORTS_SUPPORTED);
  ...
  uint32_t numHits = 0;
  HavacKernel((SequenceSegmentWord*)sequenceAsVectorIndices, sequenceLengthInSegments,
  ...
  if (numHits > NUM_HIT_REPORTS_SUPPORTED) {
    printf("ERROR: num hits %u was greater than max supported of %u\n", numHits, NUM_HIT_REPORTS_SUPPORTED);
    errorCode = -3;
    return hitReportList;
  }
  // transfer over the hit reports
  for (size_t i = 0; i < numHits; i++) {
	  TestbenchHitReport hr = TestbenchHitReport(hitReportMemory[i]);
    hitReportList.push_back(hr);
  }
...
bool compareSsvHitLists(std::vector<struct TestbenchHitReport> hardwareSsvHits, std::vector<struct SoftSsvHit> softwareSsvHits) {
  ...
  for (size_t i = 0; i < softwareSsvHits.size();i++) {
    ...
    size_t softwarePhmmPosition = softwareSsvHits[i].phmmPosition;
    size_t sequencePosition = softwareSsvHits[i].sequencePosition;
//    size_t sequenceSegmentIndex = sequencePosition / NUM_CELL_PROCESSORS;
//    size_t sequencePositionInSegment = sequencePosition - (sequenceSegmentIndex * NUM_CELL_PROCESSORS);
//    size_t softwareGroupIndex = sequencePositionInSegment / CELLS_PER_GROUP;
    ...
    for (auto& hardwareHit : hardwareSsvHits) {
      ...
      if ((hardwareHit.phmmPosition == softwarePhmmPosition) && (hardwareHit.sequencePosition == sequencePosition)) {
    	  foundMatchingHitReport = true;
    	  break;
//        //check to see if the group agrees, too
//        if (hardwareHit.groupsPassingThreshold.test(softwareGroupIndex)) {
...
//        		  softwarePhmmPosition, sequenceSegmentIndex, softwareGroupIndex);
//          allTestsPass = false;
//          break;
//        }
//        break;
    ...
    if (!foundMatchingHitReport) {
    	printf("TEST FAIL: could not match software hit @ p= %u, s=%u to a corresponding hardware report\n", softwarePhmmPosition, sequencePosition);
//      printf("TEST FAIL: software hit @ p=%zu, s=%zu did not have a corresponding hardware hit report\n", softwarePhmmPosition, sequenceSegmentIndex);
      allTestsPass = false;
    }
  }
...
//      printf("TEST FAILURE: hardware hit report @ s=%zu p=%zu had no groups asserted.\n", hardwareHit.sequenceSegmentIndex, hardwareHit.phmmPosition);
//      allTestsPass = false;
//    }
	  bool foundMatchingSoftwareHit = false;
	  for(auto &softwareHit: softwareSsvHits){
		  if(softwareHit.phmmPosition == hardwareHit.phmmPosition && softwareHit.sequencePosition == hardwareHit.sequencePosition){
...
//  	  fflush(stdout);
//      if (hardwareHit.groupsPassingThreshold.test(groupIndex)) {

//        bool foundMatchingSoftwareHit = false;
//        for (auto& softwareHit : softwareSsvHits) {
//          size_t sequencePosition = softwareHit.sequencePosition;
//          size_t sequenceSegmentIndex = sequencePosition / NUM_CELL_PROCESSORS;
//          size_t sequencePositionInSegment = sequencePosition - (sequenceSegmentIndex * NUM_CELL_PROCESSORS);
//          size_t softwareGroupIndex = sequencePositionInSegment / CELLS_PER_GROUP;
//
////          if ((softwareHit.phmmPosition == hardwareHit.phmmPosition) && (sequenceSegmentIndex == hardwareHit.sequenceSegmentIndex) &&
...
////            hardwareHit.sequenceSegmentIndex, hardwareHit.phmmPosition, groupIndex);
////          allTestsPass = false;
////        }
//      }
    }
