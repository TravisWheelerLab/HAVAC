...
struct HmmSeqPair generateRandomHmmSeqPair(const uint32_t seqLength, const bool addFlankingRegions = false, float sequenceMutationSubProbability = 0.05f);
...
void generateRandomHmmSeqPairToFiles(const uint32_t seqLength, const char* seqSrc, const char* phmmSrc, const bool addFlankingRegions = false, float sequenceMutationSubProbability = 0.05f);
