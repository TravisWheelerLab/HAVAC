...
HitVerifier::HitVerifier(shared_ptr<FastaVector>& fastaVector, shared_ptr<P7HmmList>& phmmList)
  ...
  phmmList(phmmList) {
  this->ssvCellScores = std::make_unique<float[]>(SSV_PHMM_VERIFICATION_RANGE);
  this->verifiedHits = std::make_shared<std::vector<SsvVerifiedHit>>();
}



std::shared_ptr<std::vector<SsvVerifiedHit>>
HitVerifier::verify(std::shared_ptr<std::vector<HavacHardwareHitReport>> hits) {
  for (const auto& hit : hits) {
    verifyHit(hit);
  }

  return this->verifiedHits;
}
...
void HitVerifier::verifyHit(const struct HavacHardwareHit& hardwareHitReport) {
  ...
  for (uint32_t sequenceIndex = 0; sequenceIndex < this->fastaVector->metadata.count; sequenceIndex++) {
    ...
    if (sequenceOverlapsWithPossibleHit) {
      ...
      this->verifyWithReferenceSsv(phmmIndex, localSsvPhmmStartPosition, localPhmmSsvLength, ssvSequenceRangeBegin, ssvSequenceRangeEnd);
    }
  }
}
...
void HitVerifier::verifyWithReferenceSsv(const uint32_t hitLocatedInPhmmNumber, const uint32_t sequenceNumber, const uint32_t localSsvPhmmStartPosition,
  ...
  for (uint32_t sequencePosition = ssvSequenceRangeBegin; sequencePosition < sequenceRangeEnd; sequencePosition++) {
    ...
    for (uint32_t phmmPosition = localPhmmSsvLength - 1; phmmPosition > 0; phmmPosition--) {
      ...
      this->ssvCellScores[phmmPosition] = this->ssvCellScores[phmmPosition - 1] + matchScore;
    }
    //now compute the first cell
    float matchScore = phmmRange[sequenceSymbolAsIndex];
    this->ssvCellScores[0] = matchScore;
