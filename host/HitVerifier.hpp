...
class HitVerifier {
public:
  HitVerifier(FastaVector* fastaVector, P7HmmList* phmmList, shared_ptr<vector<int8_t>> compressedPhmmScores);
...
protected:
  ...
  /// @return the local position and phmm index, as well as the global position from the original report
  PhmmPositionInfo getPhmmIndexFromPosition(const HardwareHitReport& hardwareHitReport);
  ...
  /// @param phmmPositionInfo global and local coordinates of where the hit occurred in phmm space.
  ...
  ///   this vector.
  void verifyWithReferenceSsv(const PhmmPositionInfo phmmPositionInfo, const uint32_t sequenceNumber,
  ...
  /// @param position info for where the hit happened on the phmm, including the global position,
  ///   local position, and which phmm the hit occurred in.
  ...
  void verifyHitForGroup(const PhmmPositionInfo phmmPositionInfo, uint32_t sequenceSegmentIndex, uint32_t groupIndex,
    shared_ptr<vector<VerifiedHit>> verifiedHitList, const float desiredPValue);


  void verifySsvWalkback(const PhmmPositionInfo phmmPositionInfo, const uint32_t sequenceNumber,
